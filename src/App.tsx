...
import { Plus, PenTool as GitFork, ArrowRight, Lightbulb, FileText, Share2, Download, Maximize, ZoomIn, ZoomOut, Grid, Sparkles, Loader2, X, CircleStop as AndIcon, Undo2, Redo2 } from 'lucide-react';
...
function BranchApp() {
  ...
  const [past, setPast] = useState<Array<{ nodes: AppNode[]; edges: Edge[] }>>([]);
  const [future, setFuture] = useState<Array<{ nodes: AppNode[]; edges: Edge[] }>>([]);

  // Use refs to avoid state dependency loops with nodes and edges state
  ...
  const pushHistory = useCallback(() => {
    ...
    const nodesClone = currentNodes.map(node => ({
      ...node,
      data: { ...node.data }
    }));
    const edgesClone = currentEdges.map(edge => ({ ...edge }));

    setPast((prev) => [...prev, { nodes: nodesClone, edges: edgesClone }]);
  ...
  const undo = useCallback(() => {
    ...
    setNodes(previous.nodes.map(node => ({
      ...node,
      data: {
        ...node.data,
  ...
  }, [past, setNodes, setEdges, updateNodeLabel, handleNodeFocus]);
  ...
  const redo = useCallback(() => {
    ...
    setEdges(next.edges);
  }, [future, setNodes, setEdges, updateNodeLabel, handleNodeFocus]);
  ...
  React.useEffect(() => {
    setNodes((nds) => {
      // Check if functions are already attached to prevent unnecessary state resets
      const needsInjection = nds.some(n => !n.data?.onChange || !n.data?.onFocus);
      if (!needsInjection) return nds;
  ...
  }, [updateNodeLabel, handleNodeFocus, setNodes]);

  // Wrapped Changes to capture node/edge deletion Actions
  ...
  }, [setNodes, updateNodeLabel, handleNodeFocus, pushHistory]);
    ...
    <div className="w-full h-screen bg-[#fdfdfd] relative flex flex-col overflow-hidden">
      ...
      <nav className="h-16 w-full border-b border-gray-200 bg-white flex items-center px-6 justify-between z-50">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center text-[#1a1a1a] text-lg">
            🌿
          </div>
        ...
        <div className="flex gap-2 items-center">
          <div className="flex items-center gap-1 border border-gray-200 rounded-lg p-1 bg-white mr-2 shadow-sm">
            <button
              onClick={undo}
              disabled={past.length === 0}
              className={`p-1.5 rounded transition-colors flex items-center justify-center ${
                past.length > 0 ? 'text-gray-700 hover:bg-gray-100 cursor-pointer' : 'text-gray-300 cursor-not-allowed'
              }`}
              title="되돌리기 (Ctrl + Z)"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded transition-colors flex items-center justify-center ${
                future.length > 0 ? 'text-gray-700 hover:bg-gray-100 cursor-pointer' : 'text-gray-300 cursor-not-allowed'
              }`}
